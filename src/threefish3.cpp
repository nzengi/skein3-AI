...
Threefish3::Threefish3(const std::array<uint64_t, NUM_WORDS>& key,
...
void Threefish3::encrypt(const std::array<uint64_t, NUM_WORDS>& plaintext, 
    ...
    for (size_t d = 0; d < NUM_ROUNDS / MIX_ROUNDS; d++) {
        ...
        for (size_t i = 0; i < NUM_WORDS/4; i++) {
            simd_data[i] = _mm256_add_epi64(
                simd_data[i], 
                extended_key[(d + i) % ((NUM_WORDS + 1)/4)]
            );
        }
    ...
    for (size_t i = 0; i < NUM_WORDS/4; i++) {
        _mm256_storeu_si256(
            reinterpret_cast<__m256i*>(&ciphertext[i * 4]),
            simd_data[i]
        );
    }
}
...
void Threefish3::decrypt(const std::array<uint64_t, NUM_WORDS>& ciphertext,
    ...
    for (int d = NUM_ROUNDS / MIX_ROUNDS - 1; d >= 0; d--) {
        ...
        for (int j = MIX_ROUNDS - 1; j >= 0; j--) {
            ...
            for (size_t i = 0; i < NUM_WORDS/4; i++) {
                simd_data[i] = temp[(i + (NUM_WORDS/4 - 1)) % (NUM_WORDS/4)];
            }
        ...
        if (d % (NUM_WORDS/4) == 0) {
            __m256i tweak_vector = _mm256_set_epi64x(
                0, 0, 0, (*tweak_)[d % 3]
            );
    ...
    for (size_t i = 0; i < NUM_WORDS/4; i++) {
        _mm256_storeu_si256(
            reinterpret_cast<__m256i*>(&plaintext[i * 4]),
            simd_data[i]
        );
    }
}
...
void Threefish3::simd_mix_function(__m256i& block, int round) {
    ...
    __m256i added = _mm256_add_epi64(block, _mm256_permute4x64_epi64(block, 0xB1));
    ...
    __m256i rotated = _mm256_or_si256(
        ...
        _mm256_srli_epi64(_mm256_permute4x64_epi64(block, 0xB1), 64 - rotation)
    );
...
void Threefish3::simd_inverse_mix_function(__m256i& block, int round) {
    ...
    block = _mm256_sub_epi64(
        _mm256_xor_si256(temp, rotated),
...
void Threefish3::encrypt_chunk(const uint8_t* input, uint8_t* output,
    ...
    for (size_t i = 0; i < num_blocks; i++) {
        ...
        std::memcpy(block.data(), 
                   input + (start_block + i) * BLOCK_SIZE, 
                   BLOCK_SIZE);
        ...
        // Write result
        std::memcpy(output + (start_block + i) * BLOCK_SIZE,
                   result.data(),
                   BLOCK_SIZE);
    }
}
    ...
    for (size_t i = 0; i < input.size(); i += chunk_size) {
        ...
        results.emplace_back(
            pool.enqueue([this, &input, &output, i, current_chunk] {
                this->process_chunk(input.data() + i, 
                                  output.data() + i, 
                                  current_chunk);
            })
        );
...
void Threefish3::parallel_decrypt(const std::vector<uint8_t>& input,
    ...
    for (size_t i = 0; i < num_threads; i++) {
        ...
        size_t num_blocks = (i == num_threads - 1) ? 
                           (total_blocks - start_block) : blocks_per_thread;
        
        threads.emplace_back(&Threefish3::decrypt_chunk, this,
...
void Threefish3::benchmark(size_t data_size) {
    ...
    Threefish3 cipher(key_array, tweak);
    ...
    cipher.parallel_encrypt(input_data, output_data);
    end = std::chrono::high_resolution_clock::now();
    auto parallel_duration = std::chrono::duration_cast<std::chrono::milliseconds>(end - start);
    
    // Print results
    double serial_throughput = (data_size / 1024.0 / 1024.0) / (serial_duration.count() / 1000.0);
    double parallel_throughput = (data_size / 1024.0 / 1024.0) / (parallel_duration.count() / 1000.0);
    
    std::cout << "Processed data: " << data_size / 1024.0 / 1024.0 << " MB\n";
    std::cout << "Serial processing time: " << serial_duration.count() << " ms\n";
    std::cout << "Serial throughput: " << serial_throughput << " MB/s\n";
    std::cout << "Parallel processing time: " << parallel_duration.count() << " ms\n";
    std::cout << "Parallel throughput: " << parallel_throughput << " MB/s\n";
    std::cout << "Speedup: " << parallel_throughput / serial_throughput << "x\n";
}
...
void Threefish3::process_chunk(const uint8_t* input, uint8_t* output, size_t chunk_size) {
    ...
    for (size_t i = 0; i < num_blocks; i++) {
        ...
        if (current_block_size < BLOCK_SIZE) {
            std::memset(reinterpret_cast<uint8_t*>(block.data()) + current_block_size,
                       0,
        ...
        // Write result
        std::memcpy(output + i * BLOCK_SIZE,
                   result.data(),
                   current_block_size);
    }
}
...
void Threefish3::set_operation_mode(OperationMode mode) {
    op_mode_ = mode;
}
